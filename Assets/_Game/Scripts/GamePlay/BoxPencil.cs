...
public class BoxPencil : GameUnit
{
    ...
    public void OnInit(int n, ColorType colorType, bool hasPencil = true)
    {
        ...
        if (hasPencil)
        {
            ...
            LevelManager.Ins.currentLevel.pencils.Add(pencil);
    ...
    public void SelectPencil()
    {
        if (pencil != null)
        {
            pencil.SelectPencil(TF.position + GamePlay.Ins.selectPosition, 0.3f);
        }
    }
    ...
    {
        sprite.color = LevelManager.Ins.colorDatas.GetColor(type);
        blank.sprite.color = LevelManager.Ins.colorDatas.GetColor(type);
    }
    ...
    public int RandomColor()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            random = (int)Random.Range(0, LevelManager.Ins.currentLevel.randomColor.Count);
            if (LevelManager.Ins.currentLevel.randomColor[random] != color)
            {
                break;
            }
        }

        int type = LevelManager.Ins.currentLevel.randomColor[random];
        LevelManager.Ins.currentLevel.randomColor.RemoveAt(random);
        return type;
    }
}
